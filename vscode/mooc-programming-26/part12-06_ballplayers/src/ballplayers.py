...
def most_goals(team: list) -> str:
    sorted_team = sorted(team, key= lambda player: player.goals, reverse=True)
    return sorted_team[0].name

def most_points(team: list) -> tuple:
    sorted_team = sorted(team, key=lambda player: player.goals + player.passes, reverse=True)
    return (sorted_team[0].name, sorted_team[0].number)

def least_minutes(team: list) -> BallPlayer:
    sorted_team = sorted(team, key= lambda player: player.minutes)
